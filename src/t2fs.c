...
int main() {
	...
	MFT_registers = load_MFTArea(MFT_register_amt, MFT_start_sector); //                                  [   0-4095  ][  0-31  ] -> NR_REGISTER relativo ao MFT (+1 em relação ao geral)
					          				                      	
	print_4tupla(MFT_registers[0][0]);print_4tupla(MFT_registers[1][0]);

	int homedir_start_sector = bootBlock.blockSize * MFT_registers[0][0].logicalBlockNumber;

	char file_buffer[SECTOR_SIZE];
	read_sector(8200, file_buffer);

	struct t2fs_record bitmap;
	struct t2fs_record bitmap2;
	struct t2fs_record bitmap3;
	...
	memcpy((void*) &(bitmap2.TypeVal), 	 (void*) &file_buffer[64],  1  );
	memcpy((void*) &(bitmap2.name),          (void*) &file_buffer[65],  51 );
	memcpy((void*) &(bitmap2.blocksFileSize), (void*) &file_buffer[116], 4  );
	memcpy((void*) &(bitmap2.bytesFileSize),  (void*) &file_buffer[120], 4  );
	memcpy((void*) &(bitmap2.MFTNumber), 	 (void*) &file_buffer[124], 4  );
	...
	printf("%d\n", bitmap2.TypeVal);
	printf("%s\n", bitmap2.name);
	printf("%d\n", bitmap2.blocksFileSize);
	printf("%d\n", bitmap2.bytesFileSize);
	printf("%d\n", bitmap2.MFTNumber);

	printf("%d", sizeof(struct t2fs_record));
